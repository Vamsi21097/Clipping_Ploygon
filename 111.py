...
li = []
c =0
with fiona.open('/Users/v/Desktop/Gis/_T42QZJ_20210512T053641_B02.shp', 'r') as dst_in:
    # print(len(dst_in))
...
df = pd.DataFrame(li, columns=["utm_x", "utm_y","value"])
# print(df)
# print(df['utm_x'])
# print(to_latlon(utm_x,utm_y, 42, None, True, None))
        ...
        # with fiona.open('/Users/v/Desktop/Kshema/Poly/abcd{}.shp'.format(index), 'w', **dst_in.meta) as dst_out:
        #     dst_out.write(feature)

# polygons = glob.glob('/Users/v/Desktop/Kshema/Poly/Poly*.shp')  ## Retrieve all the .shp files
# # print(polygons)
# for polygon in polygons:
#     feat = fiona.open(polygon, 'r')
#     # print(feat)
#     command = 'gdalwarp -dstnodata -9999 -cutline {} ' \
#               '-crop_to_cutline -of GTiff ./Users/v/Desktop/Kshema/Polygons/polygonfolder/T43PER_20191103T052001.tif ./outputraster/{}.tiff'.format(polygon, 'name' )
...
for i in df.index:
    ...
    utm_lat_lon = (to_latlon(df['utm_x'][i],df['utm_y'][i], 42, None, True, None))
    # print(utm_lat_lon)
    lat.append(str(utm_lat_lon[0]))
    lon.append(str(utm_lat_lon[1]))
    value.append(df['value'][i])
...
# lat = []
# lon = [str(utm_lat_lon[1])]
# print((lat),lon)

df1 = pd.DataFrame(list(zip(value,lat, lon)),columns =['value','latitude', 'longitude'])
# print(df1)

li = []
for i in df1.index:
    li.append((df1['latitude'][i]).replace(".","")+str(df1['longitude'][i]).replace(".",""))

df1['id'] = li 

df1.to_csv('/Users/v/Desktop/Gis/jyokthi1.csv')
